...
public class QryIopWindow extends QryIop {
  ...
  /**
   *  WINDOW operator constructor with distance
  ...
  protected void evaluate () throws IOException {
    ...
    while (true) {
      ...
      //  Create a new posting that satisfies the requirement of WINDOW operator
      // by iterating through the positions and trying to build a term list that
      ...
      while (true) {
        ...
        for (Qry arg : this.args) {
          QryIop q_i = (QryIop) arg;

          if (!q_i.locIteratorHasMatch()) {
            endOfLoop = true;
            break;
          }
          int curLoc = q_i.locIteratorGetMatch();
          if (curLoc < minLoc) {
            minArg = q_i;
            minLoc = curLoc;
          }
        ...
        // test if end of matching
        if (endOfLoop || minArg == null)
          break;

        if (1 + maxLoc - minLoc > distance) {
          // Not matching
